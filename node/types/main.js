...
async function _MainMenu(msg, close)
{
    ...
    var infoTypes = 
    {
        ...
        attraction: async function(msg) 
        {
            ...
            {
                var _array          = [];
                var html = `<strong>${_User.first_name}</strong> добро пожаловать на investiR. Вы можете стать нашим партнером по привлечению инвесторов`;
                var fat = await bot.sendMessage(msg.chat.id, html, {
                    parse_mode: "HTML",
                    reply_markup: {
                        "resize_keyboard": true,
