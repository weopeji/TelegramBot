...
async function _MainMenu(msg, close)
{
    ...
    var infoTypes = 
    {
        investor: async function(msg) 
        {
            ...
            if(_User.putProject) 
            {
                ...
                var fat = await bot.sendPhoto(msg.from.id, stream, {
                    ...
                    "reply_markup": {
                        "resize_keyboard": true,
                        "keyboard": [[myInvestingText, InvestingText, "💳 Реквезиты"], [rekomendationText,"🔁 Сменить роль"], ['💁🏻 Написать в Поддержку']],
                    }
                });
                _array.push(fat.message_id);
                ...
                var fat = await bot.sendMessage(msg.chat.id, html, {
                    ...
                    reply_markup: {
                        "resize_keyboard": true, 
                        "keyboard": [["💰 Мои инвестиции", "📈 Инвестировать", "💳 Реквезиты"], ["👨‍💼 Рекомендовать","🔁 Сменить роль"], ['💁🏻 Написать в Поддержку']],
        ...
        business: async function(msg) 
        {
            ...
            var fat = await h.send_html(msg.chat.id, html, {
                "resize_keyboard": true,
                "keyboard": [["❓ Как добавить проект"], ["✍ Добавить проект", "✅ Активные проекты"], [notActiveBlock, '🔁 Сменить роль'], ['💁🏻 Написать в Поддержку']]
            });
        ...
        attraction: async function(msg) {
            if(!close)
            {
                ...
                var fat = await bot.sendMessage(msg.chat.id, html, {
                    ...
                    reply_markup: {
                        "resize_keyboard": true,
                        "keyboard": [["📊 Привлечь инвесторов", "👔 Привлечь бизнес"],["🔁 Сменить роль"], ['💁🏻 Написать в Поддержку']],
                    }
                });
                _array.push(fat.message_id);
                await h.DMA(msg, _array);
